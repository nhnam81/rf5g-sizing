...
with st.sidebar:
    ...
    st.divider()
    run_button = st.button("🚀 Calculate", use_container_width=True, type="primary")
...
with tab_sinr:
    ...
    st.dataframe(sinr_data, use_container_width=True)
...
with tab_capacity:
    if result.capacity:
        ...
        st.dataframe(cap_data, use_container_width=True)
...
with tab_qos:
    if result.qos_verification:
        ...
        st.dataframe(qos_rows, use_container_width=True)
    else:
